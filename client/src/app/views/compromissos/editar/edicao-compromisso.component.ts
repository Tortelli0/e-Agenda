...
import { TipoLocalizacaoCompromissoEnum, InserirCompromissoViewModel, CompromissoInseridoViewModel, EditarComprossimoViewModel, CompromissoEditadoViewModel } from '../models/compromisso.model';
...
export class EdicaoCompromissoComponent implements OnInit {
  ...
  public opcoesLocal = Object.values(TipoLocalizacaoCompromissoEnum).filter((v) => !Number.isFinite(v));
  ...
  constructor (
    ...
    this.form = this.fb.group({
      assunto: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(30)]],
  ...
  gravar() {
    ...
    const editarCompromissoVm: EditarComprossimoViewModel = this.form.value;
  ...
  private processarSucesso(compromisso: CompromissoEditadoViewModel): void {
    this.notificacaoService.sucesso(`Compromisso ${compromisso.assunto} editado com sucesso!`);
