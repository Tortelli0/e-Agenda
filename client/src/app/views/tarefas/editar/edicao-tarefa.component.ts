...
import { PrioridadeTarefaEnum, ItemTarefaViewModel, EditarTarefaViewModel, TarefaEditadaViewModel } from '../models/tarefa.models';
...
export class EdicaoTarefaComponent implements OnInit {
  ...
  public opcoesPrioridade = Object.values(PrioridadeTarefaEnum).filter((v) => !Number.isFinite(v));
  ...
  public adicionarItem(tituloItem: string) {
    if (tituloItem.length < 3) {
      this.notificacaoService.aviso('Por favor, insira um título com ao menos 3 caracteres.');
      return;
    }
  ...
  public alterarStatusConcluido(indiceParaConclusao: number) {
    const valorItemSelecionado: ItemTarefaViewModel = this.itens.at(indiceParaConclusao).value;
    ...
    if (itemSelecionadoEstaConcluido) {
      this.notificacaoService.aviso('Conclusão do item marcada para ser cancelada.');
  ...
  private processarSucesso(registro: TarefaEditadaViewModel): void {
    this.notificacaoService.sucesso(`Tarefa "${registro.titulo}" editada com sucesso!`);
